...
class Member {
  ...
  setStartupTask(numMembers) {
    if (this.strength < 150)
      return this.g.setMemberTask(this.name, "Train Combat");
  ...
  setTask(winChance, territory) {
    ...
    if (this.strength <= MIN_POWER)
      return this.g.setMemberTask(this.name, "Train Combat");
    if (random < JUSTICE_CHANCE)
      return this.g.setMemberTask(this.name, "Vigilante Justice");
    if (random < TRAINING_CHANCE)
      return this.g.setMemberTask(this.name, "Train Combat");
    if (territory < 1) {
      if (winChance < MIN_WIN_CHANCE)
        return this.g.setMemberTask(this.name, "Territory Warfare");
      if (random < winChance)
