...
${ANSI.font.underline}>>>Basic (16)<<<${ANSI.reset}\n`
    ...
    for (let i = 0; i < (216 / row_length); i++) {
        for (let j = 16; j <= 16 + (row_length - 1); j++) {
            ...
            output += `\u001b[48;5;${code}m${String(code).padEnd(Math.ceil(padding / 2)).padStart(padding)}`
        }
        output += `\n`
    }
    ...
    let tail_width = (ns.ui.getStyles().tailFontSize * 0.6 * padding * row_length) + 3
    let tail_height = (ns.ui.getStyles().tailFontSize * lines * ns.ui.getStyles().lineHeight) + 9
