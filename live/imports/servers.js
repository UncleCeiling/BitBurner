...
export class UtilServer {
  ...
  /** @returns {Boolean} */
  get is_mine() {
    return this.details.moneyMax > 0 && this.details.backdoorInstalled
      ? true
      : false;
  }
  /** @returns {Boolean} */
  get is_miner() {
    return this.details.maxRam >= 2 &&
      this.details.hasAdminRights &&
      !this.name.includes("hacknet")
      ? true
      : false;
  }
  /** @returns {Number} */
  ...
  /** @returns {Boolean} */
  get backdoored() {
    return this.details.backdoorInstalled;
  }
}
export class AllServers {
   ...
   */
  get mines() {
    return this.array.filter((a) => a.is_mine);
  }
  /** Returns an Array containing all miners
   * @returns {Array<UtilServer>}
   */
  get miners() {
    return this.array.filter((a) => a.is_miner);
  }
}
...
export function write_map(ns) {
  ...
  ns.tprint(
    `${ANSI.fg.magenta}Wrote ${data.length} lines to 'map.txt'${ANSI.reset}`
  );
  ...
  function iterate(server) {
    let output = [
      `${"|>".padStart(server.depth * 2, "  ")}${
        server.details.hasAdminRights ? "R" : "X"
      }${server.details.backdoorInstalled ? "B" : "X"} \`${server.name}\` ${
        server.details.requiredHackingSkill
      }`,
    ];
    if (server.children.length > 0) {
      for (let child of server.children) {
        if (child.details.purchasedByPlayer) {
          continue;
        }
...
export function server_stats(ns) {
  ...
  if (servers.length <= 0) {
    ns.tprint(`${ANSI.fg.red}No Purchased Servers${ANSI.reset}`);
    return;
