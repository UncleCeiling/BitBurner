...
export async function main(ns) {
  ...
  class Job_List {
    ...
    update() {
      ...
      const CITIES = Object.values(ns.enums.CityName)
        .map((a) => new City(a))
        .sort((a, b) => b.pop_estimate - a.pop_estimate);
      const GENERAL = ns.bladeburner
        .getGeneralActionNames()
        .map((a) => new Action("General", a));
      const CONTRACTS = ns.bladeburner
        .getContractNames()
        .map((a) => new Action("Contracts", a));
      const OPERATIONS = ns.bladeburner
        .getOperationNames()
        .map((a) => new Action("Operations", a));
      ...
      const BLACKOP =
        next_black_op != null
          ...
          : [new Action("Black Operations", "")];
      const ALL_ACTIONS =
        BLACKOP[0].name != ""
          ? BLACKOP.concat(OPERATIONS, CONTRACTS, GENERAL)
          : OPERATIONS.concat(CONTRACTS, GENERAL);
      ns.print(
        `${ANSI.fg.cyan}Checking ${
          ALL_ACTIONS.length * CITIES.length
        } jobs across ${CITIES.length} cities.${ANSI.reset}`
      );
      for (let city of CITIES) {
        ns.bladeburner.switchCity(city.name);
        for (let action of ALL_ACTIONS) {
          if (
            action.type == "Black Operations" &&
            ns.bladeburner.getNextBlackOp().rank >= ns.bladeburner.getRank()
          ) {
            continue;
          }
          let job = new Job(
          ...
          if (
            ...
            ns.bladeburner.getActionCountRemaining(job.type, job.name) < 1
          ) {
            continue;
          }
          this.jobs.push(job);
      ...
      ns.print(
        `${ANSI.fg.cyan}Found ${this.jobs.length} doable jobs.${ANSI.reset}`
      );
      return this.jobs;
    }
    /** Clears the job list */
    ...
    rep_job() {
      ...
      for (let job of this.jobs) {
        if (job.type == "General") {
          ...
          job.name == "Raid" &&
          ns.bladeburner.getCityCommunities(job.city) > 0
        ) {
          return job;
        }
        rep_jobs.push(job);
    ...
    cash_job() {
      ...
      return this.jobs
        ...
        .sort(
          (a, b) =>
            ns.bladeburner.getActionCountRemaining(b.type, b.name) -
            ns.bladeburner.getActionCountRemaining(a.type, a.name)
        )[0];
    }
     ...
     */
    chaos_job(chaos = 50) {
      // 50 is the effective floor for chaos (see src/Bladeburner/data/Constants.ts - line 31)
      if (this.jobs.length < 1) {
        ns.print(`${ANSI.fg.red}No doable jobs in job list.${ANSI.reset}`);
        return null;
      }
      for (let job of this.jobs) {
        if (
          job.name == "Diplomacy" &&
          ns.bladeburner.getCityChaos(job.city) > chaos
        ) {
          return job;
        }
      }
      ...
      if (
        ...
        try {
          ...
          if (!ns.stanek.acceptGift()) {
            ns.tprint(
              `${ANSI.fg.red}Failed to accept Stanek's Gift.${ANSI.reset}`
            );
            return;
          }
        }
  ...
  while (true) {
    ...
    if (current_job.name != "" && current_job.type != "") {
      if (
        ...
        ns.print(
          `${ANSI.fg.red}${current_job.name} too risky to continue.${ANSI.reset}`
        );
        ...
        ns.print(
          `${ANSI.fg.red}${current_job.name} ran out of contracts.${ANSI.reset}`
        );
        ns.bladeburner.stopBladeburnerAction();
      }
    }
    ...
    let next_black_op = ns.bladeburner.getNextBlackOp();
    if (next_black_op == null) {
      let working_on_achievement = new Achievements(ns).node_check(
        12,
        "CHALLENGE_BN12"
      );
      if (working_on_achievement) {
        ...
        ns.tprint(
          `${ANSI.fg.green}Bladeburner complete - Destroy Node to continue${ANSI.reset}`
        );
      }
    }
    ...
    if (
      ...
      (await do_job(job_list.black_job(), current_job))
    ) {
      continue;
    }
    // Check accuracy of data and do Field Analysis if not good, otherwise Train
    if (await improve_accuracy(current_job)) {
      continue;
    }
    // Gain Rep
    if (await do_job(job_list.rep_job(), current_job)) {
      continue;
    }
    ...
    if (
      ...
      ns.ui.closeTail();
      ns.tprint(
        `${ANSI.fg.red}Bladeburner Stopped - Not enough spare RAM on 'home',${ANSI.reset}`
      );
      ns.toast(
        "Bladeburner Stopped - Not enough spare RAM on 'home'.",
        "error"
      );
      await do_job(
        new Job(
          new Action("General", "Training"),
          new City(ns.bladeburner.getCity())
        ),
        current_job
      );
      return;
    ...
    if (await stamina_check(0.5, 0.6, current_job)) {
      continue;
    }
    // Make money
    if (await do_job(job_list.cash_job(), current_job)) {
      continue;
    }
    // Reduce Chaos
    await chaos_reduction(current_job);
    // Recruit if possible
    if (await do_job(job_list.recruit_job(), current_job)) {
      continue;
    }
  ...
  async function stamina_check(start, stop, current_job) {
    const REGENERATION = new Job(
      new Action("General", "Hyperbolic Regeneration Chamber"),
      new City(ns.bladeburner.getCity())
    );
    const TRAINING = new Job(
      new Action("General", "Training"),
      new City(ns.bladeburner.getCity())
    );
    ...
    if (stamina[0] / stamina[1] <= start) {
      while (stamina[0] / stamina[1] <= stop) {
        ns.print(
          `${ANSI.fg.yellow}Recovering Stamina: ${(
            100 *
            ((stamina[0] - stamina[1] * start) / (stamina[1] * stop))
          ).toPrecision(3)}% Complete${ANSI.reset}`
        );
        let hp = ns.getPlayer().hp;
        current_job.update();
        if (
          100 * ((stamina[0] - stamina[1] * start) / (stamina[1] * stop)) <=
          20
        ) {
  ...
  async function chaos_reduction(current_job) {
    current_job.update();
    while (await do_job(job_list.chaos_job(), current_job)) {
      ns.clearLog();
      current_job.update();
    }
  }
  ...
  async function improve_accuracy(current_job) {
    ...
    let estimate = ns.bladeburner.getActionEstimatedSuccessChance(
      "Operations",
      "Assassination"
    );
    if (black_op != null) {
      estimate = ns.bladeburner.getActionEstimatedSuccessChance(
        "Black Operations",
        black_op.name
      );
    }
    ...
    if (estimate[0] != estimate[1]) {
      ...
      ns.print(
        `${ANSI.fg.red}Couldn't find way to improve estimates. (${(
          (estimate[1] - estimate[0]) *
          100
        ).toPrecision(3)}%).\nEst. Pop. ${Math.floor(
          ns.bladeburner.getCityEstimatedPopulation(ns.bladeburner.getCity())
        ).toLocaleString()}${ANSI.reset}`
      );
      return false;
    }
