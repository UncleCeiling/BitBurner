...
export async function main(ns) {
  ...
  for (let server of Object.keys(contracts)) {
    for (let contract of contracts[server]) {
      ...
      if (!Object.keys(SOLUTIONS).includes(type)) {
        ns.tprint(
          `${ANSI.fg.yellow}${type} does not exist (${server}).${ANSI.reset}`
        );
        continue;
      }
      ...
      if (reward) {
        if (!reward.includes("No reward")) {
          ns.tprint(
            `${ANSI.fg.green}${type}: ${contract} solved - ${reward}.${ANSI.reset}`
          );
        }
        history.success++;
      } else {
        ns.tprint(
          `${ANSI.fg.red}${type}: Failed to solve ${contract}.\nData: ${data_in}\nResult: ${result}${ANSI.reset}`
        );
        history.fail++;
      }
      await ns.asleep(10);
  ...
  if (history.success > 0) {
    ns.tprint(
      `${ANSI.fg.green}Completed ${history.success} contracts.${ANSI.reset}`
    );
  }
  if (history.fail > 0) {
    ns.tprint(`${ANSI.fg.red}Failed ${history.fail} contracts.${ANSI.reset}`);
  }
