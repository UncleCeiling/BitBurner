...
export async function main(ns) {
  ...
  class MineJob {
    ...
    get hack_threads() {
      let threads = Math.floor(
        ns.hackAnalyzeThreads(this.name, this.details.moneyMax * 0.49)
      );
      return threads <= 0 ? 1 : threads;
    }
    get hack_security_increase() {
      return ns.hackAnalyzeSecurity(this.hack_threads, this.name);
    }
    get weaken_time() {
      return ns.getWeakenTime(this.name);
    }
    get weaken_hack_delay() {
      return 0;
    }
    ...
    weaken_threads(cores = 1, decrease) {
      let threads = 1;
      while (ns.weakenAnalyze(threads, cores) <= decrease) {
        threads++;
      }
      return threads;
    }
    weaken_threads_hack(cores = 1) {
      return this.weaken_threads(cores, this.hack_security_increase);
    }
    get grow_time() {
    ...
    grow_security_increase(cores = 1) {
      return ns.growthAnalyzeSecurity(
        this.grow_threads(cores),
        this.name,
        cores
      );
    }
    weaken_threads_grow(cores = 1) {
      return this.weaken_threads(cores, this.grow_security_increase(cores));
    }
    get hack_RAM() {
      return ns.getScriptRam("scripts/_hack.js") * this.hack_threads;
    }
    weaken_hack_RAM(cores = 1) {
      return (
        ns.getScriptRam("scripts/_weaken.js", "home") *
        this.weaken_threads_hack(cores)
      );
    }
    grow_RAM(cores = 1) {
      return (
        ns.getScriptRam("scripts/_grow.js", "home") * this.grow_threads(cores)
      );
    }
    weaken_grow_RAM(cores = 1) {
      return (
        ns.getScriptRam("scripts/_hack.js", "home") *
        this.weaken_threads_grow(cores)
      );
    }
    total_RAM(cores = 1) {
      return (
        this.hack_RAM +
        this.weaken_hack_RAM(cores) +
        this.grow_RAM(cores) +
        this.weaken_grow_RAM(cores)
      );
    }
  }
  class JobQueue {
    ...
    get jobs() {
      return this.mines
        .map((a) => new MineJob(a))
        .sort((a, b) => b.total_RAM() - a.total_RAM());
    }
  }
  // ===== MAIN =====
      ...
      for (let miner of miners) {
        ...
        if (
          ...
          queue.jobs
            .sort(
              (a, b) =>
                a.total_RAM(miner.details.cpuCores) -
                b.total_RAM(miner.details.cpuCores)
            )[0]
            .total_RAM(miner.details.cpuCores)
        ) {
          if (sec_queue.length > 0) {
            if (do_single_job(sec_queue[0], miner, "scripts/_weaken.js")) {
              sec_queue.shift();
            }
          } else if (grow_queue.length > 0 && grow_queue) {
            if (do_single_job(grow_queue[0], miner, "scripts/_grow.js")) {
              grow_queue.shift();
            }
          }
        }
  ...
  function do_single_job(item, miner, script) {
    ...
    if (ns.getRunningScript(script, miner.name, item.name, 0)) {
      return true;
    }
    ns.scp(script, miner.name, "home");
    if (ns.exec(script, miner.name, threads, item.name, 0) > 0) {
      ns.print(
        `${ANSI.fg.green}'${script.replace("scripts/", "")}'|${ns.formatRam(
          threads * ns.getScriptRam(script, "home")
        )}|${ANSI.fg.cyan}${miner.name} >> ${item.name}${ANSI.reset}`
      );
    }
    return true;
  }
  ...
  function do_entire_job(item, miner) {
    ...
    if (
      ...
      ns.exec(
        "scripts/_weaken.js",
        miner.name,
        item.weaken_threads_hack(miner.details.cpuCores),
        item.name,
        item.weaken_hack_delay
      )
    ) {
      ns.print(
        `${ANSI.fg.red}Failed to execute "scripts/_weaken.js" | ${
          miner.name
        } >> ${item.name} | t=${item.weaken_threads_hack(
          miner.details.cpuCores
        )}${ANSI.reset}`
      );
      failure = true;
    }
    if (
      ...
      ns.print(
        `${ANSI.fg.red}Failed to execute "scripts/_hack.js" | ${miner.name} >> ${item.name} | t=${item.hack_threads}${ANSI.reset}`
      );
      failure = true;
    }
    if (
      ...
      ns.exec(
        "scripts/_weaken.js",
        miner.name,
        item.weaken_threads_grow(miner.details.cpuCores),
        item.name,
        item.weaken_grow_delay
      )
    ) {
      ns.print(
        `${ANSI.fg.red}Failed to execute "scripts/_weaken.js" | ${
          miner.name
        } >> ${item.name} | t=${item.weaken_threads_grow(
          miner.details.cpuCores
        )}${ANSI.reset}`
      );
      failure = true;
    ...
    if (
      ...
      ns.exec(
        "scripts/_grow.js",
        miner.name,
        item.grow_threads(miner.details.cpuCores),
        item.name,
        item.grow_delay
      )
    ) {
      ns.print(
        `${ANSI.fg.red}Failed to execute "scripts/_grow.js" | ${
          miner.name
        } >> ${item.name} | t=${item.grow_threads(miner.details.cpuCores)}${
          ANSI.reset
        }`
      );
      failure = true;
    ...
    if (!failure) {
      ns.print(
        `${ANSI.fg.magenta}RAM=${ns.formatRam(
          item.total_RAM(miner.details.cpuCores)
        )}|${ANSI.fg.cyan}${miner.name} >> ${item.name}${ANSI.reset}`
      );
    }
  }
  ...
  function check_host() {
    if (ns.getHostname() != "home") {
      ...
      ns.tprint(
        `${
          ANSI.fg.red
        }Script must be run on 'home', not '${ns.getHostname()}'.${ANSI.reset}`
      );
      ns.toast(
        `Foreman Stopped - Script must be run on 'home', not '${ns.getHostname()}'.`,
        "error"
      );
      return false;
    } else {
  ...
  const DELAY = 5;
  const PROCESSES = 10;
  const MIN_FREE_HOME_RAM =
    ns
      .ls("home", "modules/")
      .map((a) => ns.getScriptRam(a))
      .sort((a, b) => b - a)[0] + ns.getScriptRam("main.js");
  const MAX_FREE_HOME_RAM =
    ns.ls("home", "modules/").reduce((a, b) => a + ns.getScriptRam(b), 0) +
    ns.getScriptRam("main.js");
  // Repeat ad-nauseam
  while (true) {
    await ns.asleep(100);
    ...
    while (queue.length > 0) {
      ...
      for (let miner of miners) {
        ...
        if (item.name in Object.keys(working)) {
          job.threads = job.threads - working[item.name].threads;
        }
        ...
        if (item.name in Object.keys(working)) {
          ...
          if (working[item.name].end < end_time) {
            working[item.name].end = end_time;
          }
        } else {
          working[item.name] = {
            threads: job.threads,
            end: Date.now() + job.time,
          };
        }
        // Run the job
        run_job(job, miner);
      ...
      ns.print(
        `${ANSI.fg.magenta}${queue.length} jobs in the queue.${ANSI.reset}`
      );
      ns.print(
        `${ANSI.fg.magenta}Next job in queue: ${
          get_job_details(queue[0]).host
        } | ${get_job_details(queue[0]).script}${ANSI.reset}`
      );
      ns.print(`${ANSI.fg.magenta}Sleeping for ${DELAY} secs...${ANSI.reset}`);
      await ns.asleep(DELAY * 1000);
    }
    ns.print(
      `${ANSI.fg.magenta}Refreshing queue in ${DELAY} secs...${ANSI.reset}`
    );
    await ns.asleep(DELAY * 1000);
  }
  ...
  function run_job(job, miner) {
    ...
    if (success > 0) {
      ns.print(
        `${ANSI.fg.green}${job.script}|t=${job.threads}|${ANSI.fg.cyan}${miner} >> ${item.name}${ANSI.reset}`
      );
    } else {
      ns.print(
        `${ANSI.fg.red}Failed to execute ${job.script} | ${miner} >> ${item.name} | t=${job.threads}${ANSI.reset}`
      );
    }
  }
  ...
  function get_weaken_threads(server) {
    ...
    let threads = 1;
    while (true) {
      if (ns.weakenAnalyze(threads) > sec_diff) {
        return threads;
      } else {
        threads++;
      }
  ...
  function get_hack_threads(server) {
    return Math.ceil(
      ns.hackAnalyzeThreads(server.hostname, server.moneyAvailable / 4)
    );
  }
  ...
  function get_job_details(mine) {
    ...
    if (
      ...
      ns.formulas.hacking.hackChance(server, ns.getPlayer()) == 1
    ) {
  ...
  function get_queue() {
    ...
    for (let mine of mines) {
      if (mine in Object.keys(working)) {
        continue;
      } else {
        queue.push(mine);
      }
  ...
  function too_many_processes(miner) {
    ...
    if (miner == HOST) {
      if (ns.ps(HOST).length >= PROCESSES) {
        return true;
      } else {
        return false;
      }
