...
export async function main(ns) {
  ...
  for (let server of servers) {
    for (let result of ns.scan(server)) {
      if (ns.getServer(result).purchasedByPlayer) {
        continue;
      }
      servers.add(result);
    }
  ...
  if (toRoot.length > 0) {
    ...
    for (let root of toRoot) {
      ...
      if (required > ns.getHackingLevel()) {
        ns.print(`WARN - Skipping ${root}. (Needs hacking level ${required})`);
        continue;
      }
  ...
  // Otherwise, report all-clear
    ...
    ns.tprint(`INFO - All ${serverDetails.length} servers have been rooted.`);
  }
}
