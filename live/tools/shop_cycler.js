...
export async function main(ns) {
  ...
  if (!(duration > 0)) {
    ns.tprint(
      `${ANSI.bg.red}"${duration}" is not a valid argument (please enter an integer larger than 1)${ANSI.reset}`
    );
    return;
  }
  ...
  for (let complete_cycles = 1; complete_cycles <= cycles; complete_cycles++) {
    ...
    if (complete_cycles % 10 == 0) {
      ...
      let message = `${ANSI.fg.cyan}${Math.floor(
        (complete_cycles / cycles) * 100
      )}% completed | Approx ${estimate} seconds until cycler ends${
        ANSI.reset
      }`;
      if (
        complete_cycles % 100 == 0 &&
        Math.floor((complete_cycles / cycles) * 100) != 100
      ) {
        ns.tprint(message);
      } else {
        ns.print(message);
