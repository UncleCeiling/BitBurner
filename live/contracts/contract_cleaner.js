...
export async function main(ns) {
  ...
  for (let server of servers) {
    ...
    for (let file of contracts) {
      if (ns.rm(file, server)) {
        ns.print(`SUCCESS - Deleted ${file} from ${server}`);
        history.deleted++;
      } else {
        ns.print(`FAIL - Could not delete ${file} from ${server}`);
        history.failed++;
      }
    }
    ns.asleep(100);
  ...
  if (history.deleted > 0 || history.failed > 0) {
    ns.tprint(
      `${ANSI.fg.cyan}${history.deleted} contracts deleted from ${history.servers} servers.\n${history.failed} contracts failed to delete.${ANSI.reset}`
    );
  }
}
