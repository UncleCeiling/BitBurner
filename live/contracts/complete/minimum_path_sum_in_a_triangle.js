...
export default function minimum_path_sum_in_a_triangle(data_in) {
  ...
  for (let row in original_tri) {
    cost_tri.push([]);
    for (let item in original_tri[row]) {
      cost_tri[row][item] = null;
    }
  }
  ...
  cost_tri.reverse();
  // Fill first row
  for (let item in original_tri[0]) {
    cost_tri[0][item] = original_tri[0][item];
  }
  for (let row = 0; row < original_tri.length; row++) {
    for (let i = 0; i < original_tri[row].length; i++) {
      if (i == original_tri[row].length - 1 && i >= 0) {
        continue;
      }
