...
export async function main(ns) {
  ...
  if (arg_present) {
    ...
    if (invalid_arg_type || invalid_arg_value) {
      ns.tprint(
        `${ANSI.fg.red}${ARGUMENT} is not a valid argument (must be an integer >= 1)${ANSI.reset}`
      );
      return;
    } else {
  ...
  while (true) {
    ...
    ns.print(
      `${ANSI.fg.cyan}Taking a break for ${break_secs} seconds.${ANSI.reset}`
    );
    ns.tprint(
      `${ANSI.fg.magenta}Taking a break for ${break_secs} seconds.${ANSI.reset}`
    );
