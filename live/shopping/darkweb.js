...
export async function main(ns) {
  ...
  if (!ns.singularity.purchaseTor()) {
    ns.tprint(`${ANSI.fg.red}TOR router not owned${ANSI.reset}`);
    return;
  ...
  while (min < ns.getServerMoneyAvailable("home")) {
    ...
    if (ns.getServerMoneyAvailable("home") > Math.min(...programCosts)) {
      ...
      ns.print(
        `${
          ANSI.fg.yellow
        }Not enough funds to purchase ${buyChoice} - Need $${Math.min(
          min
        ).toLocaleString()}${ANSI.reset}`
      ); // Otherwise we are out of cash
    }
    await ns.asleep(10);
  }
}
