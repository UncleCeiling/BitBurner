...
export async function main(ns) {
    ...
    report() {
      if (history.spent > 0) {
        ...
        if (history.contracts > 0) {
          list.push(`${history.contracts} contracts`);
        }
        if (history.bladeburner_sp > 0) {
          list.push(`${history.bladeburner_sp * 10} skill points`);
        }
        if (history.sec_down > 0) {
          list.push(`${history.sec_down} security downgrades`);
        }
        if (history.money_up > 0) {
          list.push(`${history.money_up} money upgrades`);
        }
        if (history.money > 0) {
          list.push(`${history.money} money`);
        }
        let data = list.join(", ");
        ...
        ns.tprint(
          `${ANSI.fg.green}Bought ${data} for ${history.spent} hashes.${ANSI.reset}`
        );
      }
    }
  }
  ...
  while (buy_hash_upgrade("Generate Coding Contract")) {
    history.contracts++;
  } // Attempt to buy Contracts
  if (
    ns.bladeburner.inBladeburner() &&
    ns.bladeburner.getSkillUpgradeCost("Overclock") < Infinity
  ) {
    while (buy_hash_upgrade("Exchange for Bladeburner SP")) {
      history.bladeburner_sp++;
    }
  ...
  function buy_hash_upgrade(upgrade, target = "") {
    ...
    if (cost > hashes) {
      ...
      ns.hacknet.spendHashes(upgrade, target);
    } else {
  ...
  function server_upgrade() {
    let servers = new AllServers(ns).array.filter(
      (a) => a.details.backdoorInstalled
    );
    for (let server of servers.sort(
      (a, b) => b.details.moneyMax - a.details.moneyMax
    )) {
      if (server.details.minDifficulty > 1) {
        while (buy_hash_upgrade("Reduce Minimum Security", server.name)) {
          history.sec_down++;
        }
    ...
    for (let server of servers.sort(
      ...
      while (buy_hash_upgrade("Increase Maximum Money", server.name)) {
        history.money_up++;
