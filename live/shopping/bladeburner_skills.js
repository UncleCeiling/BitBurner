...
export async function main(ns) {
      ...
      if (
        ...
        while (success) {
          ...
          if (ns.bladeburner.upgradeSkill("Overclock")) {
            ns.tprint(
              `${ANSI.fg.green}Bought Overclock for ${cost} points (Level: ${
                level + 1
              })${ANSI.reset}`
            );
          } else {
        ...
        skill_levels.push({ name: skill, level: level, cost: cost });
      }
    }
    success = false;
    for (let item of skill_levels.sort((a, b) => a.level - b.level)) {
      if (item.name == "Overclock" && item.level >= 90) {
        continue;
      }
      points = get_skill_points();
      if (points > item.cost) {
        if (ns.bladeburner.upgradeSkill(item.name)) {
          ns.tprint(
            `${ANSI.fg.green}Bought ${item.name} for ${
              item.cost
            } points (Level: ${item.level + 1})${ANSI.reset}`
          );
          success = true;
        } else {
          ns.tprint(
            `${ANSI.fg.red}Failed to buy ${item.name} for ${item.cost} points (Current points: ${points})${ANSI.reset}`
          );
        }
      }
    }
  ...
  function get_skill_points() {
    return ns.bladeburner.getSkillPoints();
  }
}
