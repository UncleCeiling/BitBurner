...
export async function main(ns) {
  ...
  for (let sleeve of sleeves) {
    ...
    if (purchasable_augs(sleeve).length == 0) {
      continue;
    }
    // for each augment in the list of augments available to purchase (sorted by descending cost)
    for (let augment of purchasable_augs(sleeve)) {
      buy_augment(sleeve, augment.name, augment.cost);
    }
  }
  print_history();
  ...
  function purchasable_augs(sleeve_num) {
    return ns.sleeve
      .getSleevePurchasableAugs(sleeve_num)
  ...
  function buy_augment(sleeve_num, augment_name, augment_cost) {
    if (ns.getServerMoneyAvailable("home") > augment_cost) {
      if (ns.sleeve.purchaseSleeveAug(sleeve_num, augment_name)) {
        ...
        ns.print(
          `${ANSI.fg.green}Sleeve ${sleeve_num} bought ${augment_name} for ${augment_cost}.${ANSI.reset}`
        );
        return true;
      } else {
        ns.print(
          `${
            ANSI.fg.red
          }Sleeve ${sleeve_num} could not purchase ${augment_name} for ${augment_cost} (Bank:${ns.getServerMoneyAvailable(
            "home"
          )}).${ANSI.reset}`
        );
        return false;
  ...
  function print_history() {
    if (history.augments > 0) {
      ns.tprint(
        `${ANSI.fg.green}Bought ${history.augments} augments for $${history.money}.${ANSI.reset}`
      );
    }
  }
